        ...
        match typeinfo {
            ...
            Struct{members, ..} => {
                ...
                if currentmember.is_some() {
                    TypeInfoIter::StructLike{struct_iter, current_member: currentmember, member_iter: None}
                } else {
                    TypeInfoIter::NotIterable
                }
            }
            Array { ref size, dim, ref stride, arraytype } => {
                TypeInfoIter::Array{size: *size, dim, stride: *stride, arraytype, position: 0, item_iter: None}
            }
            _ => {
                TypeInfoIter::NotIterable
            }
...
impl<'a> Iterator for TypeInfoIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            ...
            TypeInfoIter::StructLike{struct_iter, current_member, member_iter} => {
                    ...
                    if member_iter.is_none() {
                        *member_iter = Some(Box::new(TypeInfoIter::new(typeinfo)));
                        ...
                        if let Some((member_name, member_typeinfo, member_offset)) = member {
                            Some((format!(".{}{}", name, member_name), member_typeinfo, offset + member_offset))
                        } else {
            ...
            TypeInfoIter::Array { size, dim, stride, position, arraytype, item_iter} => {
                    ...
                    if item_iter.is_none() {
                        ...
                        *item_iter = Some(Box::new(TypeInfoIter::new(arraytype)));
                        ...
                        if let Some((item_name, item_typeinfo, item_offset)) = item {
                            Some((format!("{}{}", idxstr, item_name), item_typeinfo, offset + item_offset))
                        } else {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some((varname, VarInfo{address, typeref})) = self.current_var {
            if self.type_iter.is_none() {
                ...
                if let Some((type_name, type_info, offset)) = self.type_iter.as_mut().unwrap().next() {
                    Some((format!("{}{}", varname, type_name), Some(type_info), *address + offset))
                } else {
...
fn test_varinfo_iter() {
    ...
    variables.insert("var_a".to_string(), VarInfo { address: 1, typeref: 0 });
    variables.insert("var_b".to_string(), VarInfo { address: 2, typeref: 0 });
    variables.insert("var_c".to_string(), VarInfo { address: 3, typeref: 1 });
